...
class VIGOR(torch.utils.data.Dataset):
    def __init__(self, mode = '', root = './VIGOR/', same_area=True, print_bool=False, polar = '', args=None):
        ...
        if same_area:
            self.train_city_list = ['Chicago']  #'NewYork', 'Seattle', 'SanFrancisco',
            self.test_city_list = ['Chicago']  #'NewYork', 'Seattle', 'SanFrancisco', 
        else:
    ...
    def __getitem__(self, index, debug=False):
        # print('self.train_sat_cover_dict:',self.train_sat_cover_dict)
        ...
        if 'train' in self.mode:
            ...
            # output gt
            [col_offset, row_offset] = self.train_delta[idx, 0]  # delta = [delta_lat, delta_lon]
            row_offset_resized = (row_offset / 640 * self.sat_ori_size[0]).astype(np.int32)
            col_offset_resized = (col_offset / 640 * self.sat_ori_size[0]).astype(np.int32)
            
            # Creating x and y grids
            x = np.linspace(-self.sat_ori_size[0] / 2 + row_offset_resized, self.sat_ori_size[0] / 2 + row_offset_resized, self.sat_ori_size[0])
            y = np.linspace(-self.sat_ori_size[0] / 2 - col_offset_resized, self.sat_ori_size[0] / 2 - col_offset_resized, self.sat_ori_size[0])
            x, y = np.meshgrid(x, y)

            # Calculate the distance matrix  indexing='xy'
            d = np.sqrt(x * x + y * y)
            
            sigma, mu = 4, 0.0
            # Generate the ground truth of the Gaussian distribution GT
            batch_gt = np.zeros([1, self.sat_ori_size[0], self.sat_ori_size[1]], dtype=np.float32)
            batch_gt[0, :, :] = np.exp(-((d - mu) ** 2 / (2.0 * sigma ** 2)))

            return img_query, img_reference, torch.tensor(idx), torch.tensor(idx), torch.tensor(self.train_delta[idx, 0]), 0, batch_gt
        ...
        elif 'test_q_r' in self.mode:
            img_query = Image.open(self.test_list[index])
            img_query = self.transform_query(img_query)
            img_reference = Image.open(self.test_sat_list[self.test_label[index][0]]).convert('RGB')
            img_reference = self.transform_reference(img_reference)
            return img_query, img_reference, torch.tensor(index), torch.tensor(self.test_delta[index, 0])
        else:
